...
void ControlledProcessManager::handleProcessStats(std::pair<std::chrono::system_clock::time_point, process_manager::ProcessStats> timePstsPair)
{
    ...
    for (auto& processStat : timePstsPair.second.processStats) {
        ...
        if (controlledRobot != nullptr) {
            ...
            controlledRobot->handleProcessStat(timePstsPair.first, processStat, processStat.robotID);
        }
    }
}
