    ...
    if (execEntry != this->executableMap.end()) {
        ...
        if (executableMetaData != nullptr) {
            auto mapIter = this->executableMap.emplace(
                    execId, new ManagedExecutable(executableMetaData, ExecutableMetaData::NOTHING_MANAGED, this->name, this->procMan));
...
/**
 *	This method is the internal way to start a new process. The method changeDesiredState is the way to let the
 *ManagedRobot object launch a process.
 * @param execName
 * @param execid
 */
