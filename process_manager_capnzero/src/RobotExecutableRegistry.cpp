...
const essentials::Identifier* RobotExecutableRegistry::getRobotId(std::string& robotName, const uint8_t* idBytes, int idSize, uint8_t type)
{
    auto agentID = this->getIDFromBytes(idBytes, idSize, type);
...
const essentials::Identifier* RobotExecutableRegistry::getRobotId(const uint8_t* idBytes, int idSize, uint8_t type)
{
    auto agentID = this->getIDFromBytes(idBytes, idSize, type);
    auto agentEntry = this->robotMap.find(agentID);
...
const essentials::Identifier* RobotExecutableRegistry::addRobot(string agentName)
{
    ...
    try {
        ...
        agentID = this->getID<int>(tmpID, essentials::Identifier::UUID_TYPE);
    } catch (const std::runtime_error* e) {
        agentID = nullptr;
        do {
            // generates random ID
            agentID = this->generateID(4);
    ...
    if (paramSets->size() > 1 || paramSets->at(0) != "NONE") {
        for (string paramSetKeyString : (*paramSets)) {
            try {
                ...
                std::vector<std::string> paramSetValues = sc["ProcessManaging"]->getList<string>(
