...
RawSensorData::RawSensorData(srg::SRGWorldModel* wm)
{
    ...
    this->perceptionsBuffer =
            new supplementary::InfoBuffer<srg::sim::containers::Perceptions>(sc["SRGWorldModel"]->get<int>("Data.Perception.BufferLength", NULL));
}
...
supplementary::InfoBuffer<agent::SpeechAct>& RawSensorData::editSpeechActBuffer()
{
    return *this->speechActBuffer;
}
...
supplementary::InfoBuffer<srg::sim::containers::Perceptions>& RawSensorData::editPerceptionsBuffer()
{
    return *this->perceptionsBuffer;
}
...
{
    auto perceptionsInfo = std::make_shared<supplementary::InformationElement<srg::sim::containers::Perceptions>>(
            simPerceptions, wm->getTime(), perceptionsValidityDuration, 1.0);
